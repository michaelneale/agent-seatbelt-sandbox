#!/bin/bash
# Launch pi inside the sandbox.
# Requires the proxy to be running first (start-proxy.sh in another terminal).
#
# Usage:
#   ./start-pi.sh                       # interactive session
#   ./start-pi.sh -p "do something"     # run a prompt
...
echo "Starting pi in sandbox (outbound network via proxy only)"
echo "Proxy: $PROXY"
